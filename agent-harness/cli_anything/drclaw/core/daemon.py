"""
Daemon management for the VibeLab Node.js server.

State files (all under ~/.vibelab/):
  vibelab.pid          — PID of the running server process
...
Where <server_path> is resolved (highest → lowest priority):
  ...
  2. "server_path" stored in ~/.vibelab_session.json
  3. VIBELAB_SERVER_PATH environment variable
...
from .session import SESSION_FILE, _load_session_file, _save_session_file

# ---------------------------------------------------------------------------
# Paths
# ---------------------------------------------------------------------------

VIBELAB_DIR = Path.home() / ".vibelab"
PID_FILE    = VIBELAB_DIR / "vibelab.pid"
LOG_DIR     = VIBELAB_DIR / "logs"
LOG_FILE    = LOG_DIR / "server.log"


def _ensure_dirs() -> None:
    VIBELAB_DIR.mkdir(exist_ok=True)
...
def _autodetect_server_path() -> Optional[Path]:
    ...
    server/index.js.  Works because this module lives inside the VibeLab repo:
      <repo>/agent-harness/cli_anything/vibelab/core/daemon.py
    """
...
def _resolve_server_path(path_override: Optional[str] = None) -> Path:
    """Return the VibeLab installation directory (must contain server/index.js)."""
    candidates = []

    if path_override:
        candidates.append(Path(path_override))

    env_path = os.environ.get("VIBELAB_SERVER_PATH")
    if env_path:
    ...
    raise RuntimeError(
        "Cannot find VibeLab server. Pass --path /path/to/VibeLab or "
        "set VIBELAB_SERVER_PATH."
    )
...
def server_start(path_override: Optional[str] = None, port: Optional[int] = None) -> dict:
    """
    Start the VibeLab server as a background daemon.
