"""
Persistent HTTP session for the VibeLab REST API.

Token and base URL are stored in ~/.vibelab_session.json so that a single
`auth login` command is sufficient for a whole shell session (or longer).

Priority for base URL resolution (highest → lowest):
  1. url_override passed directly to VibeLab()
  2. VIBELAB_URL environment variable
  3. base_url stored in ~/.vibelab_session.json
  4. Default: http://localhost:3001

Priority for token resolution (highest → lowest):
  1. VIBELAB_TOKEN environment variable
  2. token stored in ~/.vibelab_session.json
"""
...
SESSION_FILE = Path.home() / ".vibelab_session.json"
...
class NotLoggedInError(Exception):
    ...
    def __init__(self) -> None:
        super().__init__(
            "Not logged in. Run `vibelab auth login` first, "
            "or set the VIBELAB_TOKEN environment variable."
        )
...
def _load_session_file() -> Dict[str, Any]:
    ...
    try:
        with SESSION_FILE.open("r") as fh:
            return json.load(fh)
...
class VibeLab:
    ...
    base-URL resolution for all VibeLab API calls.
    """
    ...
    def get_base_url(self) -> str:
        ...
        env_url = os.environ.get("VIBELAB_URL")
        if env_url:
    ...
    def get_token(self) -> Optional[str]:
        env_token = os.environ.get("VIBELAB_TOKEN")
        if env_token:
            return env_token
