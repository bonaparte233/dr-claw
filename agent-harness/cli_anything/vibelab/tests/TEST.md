...
| Test | What It Tests |
|------|--------------|
| `test_help_flag` | `vibelab --help` exits 0 and shows usage |
| `test_auth_status_subprocess` | `vibelab --json auth status` exits 0 with JSON output |
| `test_missing_subcommand_shows_help` | Running with no args shows help text |
| `test_auth_group_help` | `vibelab auth --help` lists login/logout/status |
