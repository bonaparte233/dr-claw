...
export async function runCompile({ projectId, mainFile, engine = 'pdflatex' }) {
  ...
  const mainArg = path.basename(absMain);
  ...
  const { cmd, args } = buildCommand(engine, outDir, mainArg);
  const needsBibPass = MULTI_PASS_ENGINES.includes(engine);

  let code;
  try {
    // Pass 1: generate .aux with \citation{} entries
    code = await runSpawn(cmd, args, mainDir, pushLog);

    if (needsBibPass) {
      const base = path.basename(mainArg, path.extname(mainArg));
      ...
      if (!useBiber) {
        try {
          const texContent = await fs.readFile(absMain, 'utf8');
          useBiber = /\\usepackage(\[.*?\])?\{biblatex\}/.test(texContent);
        } catch { /* ignore */ }
      }

      const bibCmd = useBiber ? 'biber' : 'bibtex';
      const kpsePath = `${[mainDir, projectRoot].join(path.delimiter)}${path.delimiter}`;
      const bibEnv = {
        ...process.env,
        BIBINPUTS: kpsePath,
        BSTINPUTS: kpsePath,
