...
const API_BASE = '';
function getRuntimeApiPrefix() {
  if (typeof window === 'undefined') return '';
  const runtimePrefix = (window as Window & { __LATEXLAB_API_PREFIX__?: string }).__LATEXLAB_API_PREFIX__;
  return typeof runtimePrefix === 'string' ? runtimePrefix : '';
}

const API_PREFIX_RAW = import.meta.env.VITE_LATEXLAB_API_PREFIX || getRuntimeApiPrefix() || '/api';
const API_PREFIX = API_PREFIX_RAW.startsWith('/') ? API_PREFIX_RAW : `/${API_PREFIX_RAW}`;
const NORMALIZED_API_PREFIX = API_PREFIX.replace(/\/+$/g, '') || '/api';
const BASE_PATH = (import.meta.env.BASE_URL || '/').replace(/\/$/, '');
const LANG_KEY = 'latexlab-lang';
const COLLAB_TOKEN_KEY = 'latexlab-collab-token';
const COLLAB_SERVER_KEY = 'latexlab-collab-server';

function toApiUrl(url: string) {
  if (url.startsWith('/api/')) {
    return `${API_BASE}${NORMALIZED_API_PREFIX}${url.slice(4)}`;
  }
  if (url === '/api') {
    return `${API_BASE}${NORMALIZED_API_PREFIX}`;
  }
  if (url.startsWith('/')) {
    return `${API_BASE}${url}`;
  }
...
export function getProjectBlobUrl(projectId: string, filePath: string) {
  const qs = new URLSearchParams({ path: filePath }).toString();
  return toApiUrl(`/api/projects/${projectId}/blob?${qs}`);
}
...
export async function deleteFile(id: string, filePath: string) {
  const qs = new URLSearchParams({ path: filePath }).toString();
  const res = await fetch(toApiUrl(`/api/projects/${id}/file?${qs}`), {
    method: 'DELETE',
    headers: {
...
export async function uploadFiles(projectId: string, files: File[], basePath?: string) {
  ...
  const res = await fetch(toApiUrl(`/api/projects/${projectId}/upload`), {
    method: 'POST',
    body: form,
    headers: {
      'x-lang': getLangHeader(),
...
export function runAgent(payload: {
  ...
  return request<{ ok: boolean; reply: string; suggestion: string; patches?: { path: string; diff: string; content: string }[]; sessionId?: string | null }>(`/api/agent/run`, {
    method: 'POST',
    body: JSON.stringify(payload)
  });
}
...
export async function uploadTemplate(templateId: string, templateLabel: string, file: File) {
  ...
  const lang = getLangHeader();
  const res = await fetch(toApiUrl('/api/templates/upload'), {
    method: 'POST',
...
export async function importZip(payload: { file: File; projectName?: string }) {
  ...
  const res = await fetch(toApiUrl('/api/projects/import-zip'), {
    method: 'POST',
    body: form,
    headers: {
      'x-lang': getLangHeader(),
...
export function importArxivSSE(
  ...
  return new Promise((resolve, reject) => {
    ...
    const es = new EventSource(toApiUrl(`/api/projects/import-arxiv-sse?${params.toString()}`));
...
export async function visionToLatex(payload: {
  ...
  const res = await fetch(toApiUrl('/api/vision/latex'), {
    method: 'POST',
    body: form,
    headers: {
      'x-lang': getLangHeader(),
...
export async function mineruTransferUploadPdf(jobId: string, pdfFile: File) {
  ...
  const res = await fetch(toApiUrl('/api/transfer/upload-pdf'), {
    method: 'POST',
    body: form,
    headers: {
      'x-lang': getLangHeader(),
