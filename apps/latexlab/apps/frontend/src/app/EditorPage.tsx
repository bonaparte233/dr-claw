...
type VibeLabModel = 'claude' | 'cursor' | 'codex';

type AppSettings = {
  llmEndpoint: string;
  llmApiKey: string;
  llmModel: VibeLabModel;
  searchEndpoint: string;
  searchApiKey: string;
  searchModel: VibeLabModel | '';
  visionEndpoint: string;
  visionApiKey: string;
  visionModel: VibeLabModel | '';
...
const SETTINGS_KEY = 'latexlab-settings-v1';
const VIBELAB_MODEL_VALUES: VibeLabModel[] = ['claude', 'cursor', 'codex'];
const VIBELAB_MODEL_OPTIONS: Array<{ value: VibeLabModel; label: 'Claude' | 'Cursor' | 'Codex' }> = [
  { value: 'claude', label: 'Claude' },
  { value: 'cursor', label: 'Cursor' },
  { value: 'codex', label: 'Codex' }
];
const VIBELAB_SELECTED_PROVIDER_KEY = 'selected-provider';
const VIBELAB_PROVIDER_MODEL_KEYS: Record<VibeLabModel, string> = {
  claude: 'claude-model',
  cursor: 'cursor-model',
  codex: 'codex-model'
};
const VIBELAB_DEFAULT_PROVIDER_MODELS: Record<VibeLabModel, string> = {
  claude: 'sonnet',
  cursor: 'gpt-5',
  codex: 'gpt-5.2'
};

function normalizeVibeLabModel(value: unknown, fallback: VibeLabModel): VibeLabModel {
  const normalized = typeof value === 'string' ? value.trim().toLowerCase() : '';
  return VIBELAB_MODEL_VALUES.includes(normalized as VibeLabModel) ? (normalized as VibeLabModel) : fallback;
}

function getVibeLabProviderDefault(fallback: VibeLabModel): VibeLabModel {
  if (typeof window === 'undefined') return fallback;
  const selected = window.localStorage.getItem(VIBELAB_SELECTED_PROVIDER_KEY);
  return normalizeVibeLabModel(selected, fallback);
}

function getVibeLabProviderModel(provider: VibeLabModel) {
  if (typeof window === 'undefined') return VIBELAB_DEFAULT_PROVIDER_MODELS[provider];
  const key = VIBELAB_PROVIDER_MODEL_KEYS[provider];
  const value = window.localStorage.getItem(key);
  return value && value.trim() ? value.trim() : VIBELAB_DEFAULT_PROVIDER_MODELS[provider];
}
...
const COLLAB_NAME_KEY = 'latexlab-collab-name';
...
function loadSettings(): AppSettings {
  ...
  try {
    ...
    if (!raw) {
      return {
        ...DEFAULT_SETTINGS,
        llmModel: getVibeLabProviderDefault(DEFAULT_SETTINGS.llmModel)
      };
    }
    ...
    const llmModel = normalizeVibeLabModel(parsed.llmModel, getVibeLabProviderDefault(DEFAULT_SETTINGS.llmModel));
    const searchModel = parsed.searchModel
      ? normalizeVibeLabModel(parsed.searchModel, llmModel)
      : '';
    const visionModel = parsed.visionModel
      ? normalizeVibeLabModel(parsed.visionModel, llmModel)
      : '';
    return {
      ...DEFAULT_SETTINGS,
      ...parsed,
...
type EditorPageProps = {
  ...
  embeddedOpenFile?: string;
  embeddedSourceTag?: string;
  embeddedBackTo?: string;
...
export default function EditorPage({
  ...
  const projectId = (embeddedProjectId || routeProjectId || '').trim();
  const initialOpenFile = (embeddedOpenFile || searchParams.get('openFile') || '').trim();
  const sourceTag = (embeddedSourceTag || searchParams.get('from') || '').trim().toLowerCase();
  const backTo = (embeddedBackTo || searchParams.get('backTo') || '').trim();
  const embeddedModeTag = (searchParams.get('embedded') || '').trim().toLowerCase();
  const isEmbedded = typeof embeddedMode === 'boolean'
    ? embeddedMode
    : embeddedModeTag === '1' || embeddedModeTag === 'true' || embeddedModeTag === 'yes';
  const showBackToVibeLab = sourceTag === 'vibelab' && !isEmbedded;
  const shouldLockSelectedTex = sourceTag === 'vibelab';
  ...
  const initialOpenFileRef = useRef(initialOpenFile);
  const lockedTexFileRef = useRef(shouldLockSelectedTex ? initialOpenFile : '');
  ...
  const llmRuntimeModel = useMemo(() => getVibeLabProviderModel(llmModel), [llmModel]);

  const llmConfig = useMemo(
    () => ({
      provider: llmModel,
      model: llmRuntimeModel
    }),
    [llmModel, llmRuntimeModel]
  );

  const searchLlmConfig = useMemo(() => {
    const provider = (searchModel || llmModel) as VibeLabModel;
    return {
      provider,
      model: getVibeLabProviderModel(provider)
    };
  }, [llmModel, searchModel]);

  const visionLlmConfig = useMemo(() => {
    const provider = (visionModel || llmModel) as VibeLabModel;
    return {
      provider,
      model: getVibeLabProviderModel(provider)
    };
  ...
  useEffect(() => {
    ...
    listProjects()
      .then((res) => {
        ...
        if (shouldLockSelectedTex && !lockedTexFileRef.current && current.openFile) {
          lockedTexFileRef.current = current.openFile;
        }
      })
      .catch((err) => setStatus(t('加载项目信息失败: {{error}}', { error: String(err) })));
  }, [navigate, projectId, t, shouldLockSelectedTex, isEmbedded]);
  ...
  const handleCreateInvite = useCallback(async () => {
    ...
    try {
      ...
      const link = `${base}${buildAppPath('/collab')}?token=${encodeURIComponent(res.token)}`;
  ...
  useEffect(() => {
    initialOpenFileRef.current = initialOpenFile;
    lockedTexFileRef.current = shouldLockSelectedTex ? initialOpenFile : '';
  }, [initialOpenFile, projectId, shouldLockSelectedTex]);

  const handleBackToVibeLab = useCallback(() => {
    if (backTo) {
  ...
  const refreshTree = async (keepActive = true) => {
    ...
    if (!keepActive || !activePath || !res.items.find((item) => item.path === activePath)) {
      const hasFile = (candidate: string) =>
        Boolean(candidate && res.items.find((item) => item.type === 'file' && item.path === candidate));
      const isVisiblePath = (candidate: string) =>
        candidate.split('/').every((segment) => !segment.startsWith('.'));

      const requested = initialOpenFileRef.current;
      const locked = shouldLockSelectedTex ? lockedTexFileRef.current : '';
      const requestedExists = hasFile(requested);
      const lockedExists = hasFile(locked);
      const main = res.items.find((item) => item.path.endsWith('main.tex'))?.path;
      const firstVisibleTex =
        res.items.find((item) => item.type === 'file' && item.path.toLowerCase().endsWith('.tex') && isVisiblePath(item.path))?.path || '';
      const firstVisibleFile =
        res.items.find((item) => item.type === 'file' && isVisiblePath(item.path))?.path || '';
      const firstFile = res.items.find((item) => item.type === 'file')?.path || '';
      const next =
        ...
        firstFile;
      if (next) {
        if ((requestedExists || lockedExists) && next.toLowerCase().endsWith('.tex')) {
          setMainFile(next);
        }
        if (shouldLockSelectedTex && next.toLowerCase().endsWith('.tex')) {
          lockedTexFileRef.current = next;
        }
        if (requestedExists) {
          initialOpenFileRef.current = '';
        }
        await openFile(next);
      }
    }
  ...
  const handleToggleCollab = useCallback(async () => {
    if (!activePath || !isTextPath(activePath)) return;
    if (collabEnabled) {
  ...
  }, [activePath, collabEnabled, isDirty, saveActiveFile]);
  ...
  const compile = async () => {
    ...
    try {
      if (!mainFile || !mainFile.trim()) {
        throw new Error(t('请先选择一个主 .tex 文件再编译。'));
      }
      if (!mainFile.toLowerCase().endsWith('.tex')) {
        throw new Error(t('主文件必须是 .tex 文件: {{file}}', { file: mainFile }));
      }
  ...
  const downloadPdf = useCallback(() => {
    if (!pdfUrl) return;
    const name = projectName ? projectName.replace(/\s+/g, '-') : projectId || 'latexlab';
  ...
  const sendPrompt = async () => {
    ...
    try {
      ...
      const res = await runAgent({
        ...
        sessionId: isChat ? chatSessionId : null
      });
      if (isChat && res.sessionId) {
  ...
  return (
    <div className="app-shell">
      <header className="top-bar">
        <div className="brand">
          {!isEmbedded && <div className="brand-title">LatexLab</div>}
        ...
        <div className="toolbar">
          ...
          {!isEmbedded && <Link to="/projects" className="btn ghost">{t('Projects')}</Link>}
          ...
          <div className="ios-select-wrapper">
            <button className="ios-select-trigger" onClick={(e) => {
              ...
              setEngineDropdownOpen(opening); setLangDropdownOpen(false);
          ...
          <div className="ios-select-wrapper">
            <button className="ios-select-trigger" onClick={(e) => {
              ...
              setLangDropdownOpen(opening); setEngineDropdownOpen(false);
              if (opening) { const r = e.currentTarget.getBoundingClientRect(); setTopBarDropdownRect({ top: r.bottom + 6, left: r.left, width: r.width }); }
            }}>
                  ...
                  <div className="tool-section">
                    ...
                    {plotAssetPath && (
                      <div className="vision-result">
                        ...
                        <img
                          src={getProjectBlobUrl(projectId, plotAssetPath)}
                          alt={plotAssetPath}
...
Be thorough. Read ALL .tex files before reporting. Group findings by category. If no issues are found in a category, state that explicitly.`,
                  ...
                  <div className="figure-display">
                    {selectedFigure ? (
                      selectedFigure.toLowerCase().endsWith('.pdf') ? (
                        <object data={getProjectBlobUrl(projectId, selectedFigure)} type="application/pdf" />
                      ) : (
                        <img src={getProjectBlobUrl(projectId, selectedFigure)} alt={selectedFigure} />
                      )
                    ) : (
      ...
      {(engineDropdownOpen || langDropdownOpen) && (
        <div className="topbar-dropdown-backdrop" onClick={() => { setEngineDropdownOpen(false); setLangDropdownOpen(false); }} />
      )}
            ...
            <div className="modal-body">
              <div className="field">
                ...
                <select
                  className="input"
                  value={llmModel}
                  onChange={(e) => setSettings((prev) => ({ ...prev, llmModel: normalizeVibeLabModel(e.target.value, prev.llmModel) }))}
                >
                  {VIBELAB_MODEL_OPTIONS.map((opt) => (
              ...
              <div className="field">
                ...
                <select
                  ...
                  onChange={(e) => setSettings((prev) => ({
                    ...prev,
                    searchModel: e.target.value ? normalizeVibeLabModel(e.target.value, prev.llmModel) : ''
                  }))}
                >
                  <option value="">{t('Follow main model')}</option>
                  {VIBELAB_MODEL_OPTIONS.map((opt) => (
                    <option key={opt.value} value={opt.value}>{t(opt.label)}</option>
                  ))}
                </select>
              ...
              <div className="field">
                ...
                <select
                  ...
                  onChange={(e) => setSettings((prev) => ({
                    ...prev,
                    visionModel: e.target.value ? normalizeVibeLabModel(e.target.value, prev.llmModel) : ''
                  }))}
                >
                  <option value="">{t('Follow main model')}</option>
                  {VIBELAB_MODEL_OPTIONS.map((opt) => (
