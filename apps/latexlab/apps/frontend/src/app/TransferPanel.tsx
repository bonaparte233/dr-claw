...
const VIBELAB_MODEL_VALUES = new Set(['claude', 'cursor', 'codex']);
...
export default function TransferPanel({ projectId, onJobUpdate }: TransferPanelProps) {
  ...
  const readLLMFromStorage = (): { llmEndpoint: string; llmApiKey: string; llmModel: string } => {
    try {
      ...
      const modelRaw = typeof p.llmModel === 'string' ? p.llmModel.trim().toLowerCase() : '';
      return {
        llmEndpoint: p.llmEndpoint || '',
        llmApiKey: p.llmApiKey || '',
        llmModel: VIBELAB_MODEL_VALUES.has(modelRaw) ? modelRaw : ''
      };
